...
1.  Download the `citrix-k8s-node-controller.yaml` deployment file using the following command:
    ...
    | CNI_TYPE | Mandatory | The CNI used in kubernetes cluster. Valid values: flannel,calico,canal,weave,cilium,ovn </br></br> **Note:** For OpenShift clusters using OVN-Kubernetes, set this value to `ovn`. For detailed deployment steps, see [OVN-CNI support for OpenShift](ovn-cni-openshift.md). |
